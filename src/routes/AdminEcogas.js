...
router.get('/adminecogas/TablaGeneral', (req, res) => {
    ...
    var sql = 'Select c.Nombre_sub, a.NCarpeta, a.Estado, a.id, c.ResponsableDeTarea,c.Tarea_Realizada_sub, c.Proxima_Tarea_sub, c.Fecha_Proxima_Tarea_sub, c.EtapaTarea_sub from historialdecambios c , clientes a where a.Nombre = c.Nombre_sub AND (DATE_SUB(CURDATE(), interval 7 day)) <= c.Fecha_Proxima_Tarea_sub '; //AND CURDATE()>= c.Proxima_Tarea_sub   Permite mostrar las fechas proximas a vencerse. ';
    res.locals.moment = moment;
    connection.query(sql, (error, results) => {
        if (error) console.log( error);
